...
const (
	defaultTbspPrefix = "/" + postgres.DefaultTablespace + "/"
	customTbspPrefix  = "/" + postgres.TablespaceFolder + "/"
	systemIDLimit     = 16384
)
...
func (p *ExtractProviderDBSpec) Get(
	...
	interpreter, err := p.getTarInterpreter(dbDataDir, segBackup, filesToUnwrap, createNewIncrementalFiles)
	if err != nil {
		return nil, nil, "", err
	}
	tarsToExtract, pgControlKey, err := p.FilesToExtractProviderImpl.Get(segBackup, filesToUnwrap, skipRedundantTars)
...
func (t *ExtractProviderDBSpec) getTarInterpreter(dbDataDir string, backup SegBackup,
...
func (p *ExtractProviderDBSpec) makeFullRestoreDatabaseMap(databases []string, names postgres.DatabasesByNames) (map[int]bool, error) {
